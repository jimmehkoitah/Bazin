-- Bazin discovery engine: system-of-record schema (v0.1)
-- Postgres 16 with pgvector and PostGIS. Observations are immutable; business facts are derived.
...
create table businesses (
  ...
  description               text,                -- our own generated summary, not copied text
  text_embedding            vector(1024),
  image_embedding           vector(512),         -- mean of media embeddings
...
create index businesses_name_trgm on businesses using gin (canonical_name gin_trgm_ops);
create index businesses_text_emb on businesses using hnsw (text_embedding vector_cosine_ops);
create index businesses_image_emb on businesses using hnsw (image_embedding vector_cosine_ops);
...
create table identities (
  ...
  match_evidence   jsonb,                        -- e.g. {"shared_phone": "...", "bio_link": "..."}
  last_crawled_at  timestamptz,
  unique (platform, url)
);
create index identities_business on identities (business_id);
...
create table locations (
  ...
  h3_r7         text,                            -- H3 cell at resolution 7 for fast local narrowing
  confidence    real,
  evidence_observation_id uuid
);
create index locations_geom on locations using gist (geom);
create index locations_h3 on locations (h3_r7);
create index locations_country on locations (country, kind);
...
create table contact_channels (
  ...
  value              text not null,
  normalized_value   text,                       -- E.164 for numbers, lowercase for emails
...
create table observations (
  ...
  mentions           text[],
  language           text,
  engagement         jsonb,                      -- as seen: {"likes":..,"comments":..,"views":..}
  raw_ref            text,                       -- pointer to the raw payload store (e.g. scraper dataset id), not content
  acquisition_method acquisition_method not null,
  source_policy_id   int references source_policies(id),
  extractor_version  text,
  content_hash       text,
  unique (platform, source_url, observed_at)
);
create index observations_business on observations (business_id, published_at desc);
create index observations_identity on observations (identity_id);
...
create index media_phash on media_refs (phash);
create index media_emb on media_refs using hnsw (image_embedding vector_cosine_ops);

-- Controlled vocabulary. Search terms map to concepts; observations are tagged with concepts.
create table concepts (
  id              text primary key,              -- e.g. 'garment.grand_boubou', 'fabric.getzner.super_magnum'
...
create table concept_labels (
  ...
  label       text not null,
  language    text not null,                     -- 'fr','en','wo','ha','yo','bm','it','misspelling'
  market      text,                              -- 'SN','ML','NG','diaspora', null = any
  weight      real not null default 1.0,
  primary key (concept_id, label, language)
);
create index concept_labels_label on concept_labels (lower(label));
...
create table prices (
  ...
  usd_equivalent      numeric,                   -- converted at observed_at
  observed_at         timestamptz not null
);
create index prices_business on prices (business_id, price_type);
...
create table evidence (
  ...
  confidence       real,
  observed_at      timestamptz not null default now(),
  collected_by     text                          -- 'pipeline', 'cotera:<agent>', 'manual'
);
create index evidence_business on evidence (business_id, kind);
...
create table scores (
  ...
  freshness           real,
  total               real not null,
  inputs              jsonb not null,            -- snapshot of the features used
  reasons             text[],
  caveats             text[],
  computed_at         timestamptz not null default now()
);
create index scores_business_latest on scores (business_id, computed_at desc);
...
create table clusters (
  ...
  label       text,
  description text,
  hue         smallint check (hue between 0 and 359),   -- stable colour for UI coding
  centroid    vector(64),
  size        int,
  created_at  timestamptz not null default now()
);

create table business_clusters (
  business_id     uuid references businesses(id) on delete cascade,
...
create table search_terms (
  ...
  concept_ids   text[],
  source        text not null,                   -- 'autocomplete','paa','trends','tiktok_suggest','query_log','manual'
  volume_proxy  numeric,
  observed_at   timestamptz not null default now(),
  unique (term, language, market, source)
);

-- Every consumer query from day one. This is the raw material for the vendor-facing search-term product.
...
create table claims (
  ...
  claimant_email       text,
  claimant_phone       text,
  verification_method  text,                     -- 'dm_code','whatsapp_code','website_meta','manual'
  verified_at          timestamptz,
